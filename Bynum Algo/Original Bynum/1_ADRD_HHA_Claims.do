...
################################################################################
#
# Purpose           : Get Home Health Agency file (HHA) ADRD claims for a single 
#                     year of data using Julie Bynum definitions
#
# Input files       : Single year of Home Health Agency (HHA) Data Claims File (.sas7bdat format; not including the Revenue Center File)
#
# Output files      : diag_adrd_HHA_&year    : Beneficiary-Diagnosis level file with all ADRD Dx. codes found in claims
#                     servdt_adrd_HHA_&year  : Beneficiary-Service Date level file derived from diag_adrd_HHA_&year
#                     bene_adrd_HHA_&year    : Beneficiary-level file derived from servdt_adrd_HHA_&year
#
...
local file_name = "hhaclms18.sas7bdat" /* update with name of data file (include .sas7bdat extention) */
...
cd "`output_folder'"
save "diag_adrd_HHA_`data_year'.dta", replace
...
save "servdt_adrd_HHA_`data_year'.dta", replace
...
collapse (count) HHA_ADRD_CLMS=NUM_ADRD_DX, by(BENE_ID ADRD_DX)

generate HHA_ADRD_DX = ADRD_DX

label variable HHA_ADRD_CLMS "Total number of ADRD claims by bene_id in HHA"
label variable HHA_ADRD_DX "In HHA, presence of ADRD diagnosis. 1/0"

tab1 ADRD_DX HHA_ADRD_DX HHA_ADRD_CLMS

save "bene_adrd_HHA_`data_year'.dta", replace
